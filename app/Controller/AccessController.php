     ...
     public function ajax_login() {
         ...
         if (is_null($this->Auth->user())) {//Si no esta ya logueado
             if ($this->request->is('post')) {
                ...
                preg_match_all('~\b(Windows|Trident|rv)\b~i', $_SERVER['HTTP_USER_AGENT'], $matches);
                $skipCaptcha = is_array($matches) && isset($matches[0]) && count($matches[0])==3;
                //mail('elalbertgd@gmail.com','test',print_r($skipCaptcha, true)); 
                if ($skipCaptcha || (isset($data_login['s3capcha']) && $this->Session->check('s3capcha') && $data_login['s3capcha'] == $this->Session->read('s3capcha') && $data_login['s3capcha'] != '')) {
                     ...
                     if (!empty($usuario) && $this->Auth->login($usuario)) {
                         ...
                         if ($usuario['UsuariosEmpresa']['fingerprint'] == $fingerprint) {*/
                            ...
                            $response['needsAcceptEula'] = false;


                            $error_login = false;

                            if($usuario['UsuariosEmpresa']['eula_ok']>0){ //already eula accepted
                                ...
                                if($usuario['UsuariosEmpresa']['fingerprint'] == ""){ //user has no auth fingerprint for his device and fingerprint not comes correctly
                                    if($forceLogin === false){
                                        ...
                                        $this->Session->write('fingerprint', $fingerprint);
                            ...
                            }else{ //force to accept eula
                                $error_login = true;
                                $response['needsAcceptEula'] = true;
                                $secureUserParams = $this->Core->encodeParams($usuario['UsuariosEmpresa']['id']);
                                $response['userId'] = implode('||', $secureUserParams);
                            }
                            ...
                            if (!$error_login) {
                                ...
                                if($usuario['UsuariosEmpresa']['i_group_id'] <= IGroup::REGION_MANAGER){ //Country, Market y Master, no se checan
                                    ...
                                    if(!is_numeric($country_code_tracked) && strlen($country_code_tracked)==2) { //if is text and value length is 2, then is valid country code
                                        ...
                                        if(!in_array($country_code_tracked,$countryExclussion)){ //Checar si el codigo trackedado no esta dentro de las exclusiones
                                            if($country_code_user=="CENAM"){ //la region del usuario es cenam, checar si el pais desde donde se logueo pertenece a cenam
                                                ...
                                                //Us And canada will be the same code
                                                $createNotification = in_array($country_code_user, ['US','CA']) && in_array($country_code_tracked,['US','CA']) ? false : true;
    ...
    public function logout() {
        ...
        if ($this->Auth->loggedIn()) {//Si efectivamente esta logueado
            ...
            if (!empty($usuario)) {
                ...
                $this->Session->delete('permissionsRole');
...
//            $this->redirect($redirects[$group_user]);
    ...
    public function ax_set_position() {
        ...
        if ($this->request->is('post')) {
            ...
            if($lat==0 && $lon==0)
            {
              mail("elalbertgd@gmail.com, admin@contiplus.net","Falla en nueva api de request de coordenadas aproximadas","La api https://flourishworks-webutils.appspot.com/req ha fallao tio.");
            }
