 ...
 *
 *     @file list_reporting_conveyors.php
 *     View layer for action listReportingConveyors of controller
 *
 *     @project    Contiplus
    ...
    <body>
        ...
        <div id="content">            
            <div class="center-aligned">
                <p class="title-download"><?php echo __('Lista de transportadores'); ?></p>
                <p class="title-conveyor-download"><?php echo $company['Empresa']['name']; ?></p>
                <table>
                <?php 
                    $data = '<thead><tr><th class="created">'.__('Creado', true).'</th><th class="conveyor">'.__('Transportador', true).'</th><th class="title">'.__('Banda recomendada', true).'</th><th class="created_by">'.__('Medida', true).'</th></tr></thead>';
                    $data .= '<tbody>';
                    if(!empty($conveyors)){
                        foreach($conveyors AS $id_conveyor => $conveyor){
                            $fecha_creacion = $this->Utilities->transformVisualFormatDate($conveyor['creada'], true);
                            $data .= '<tr><td class="center-aligned">'.$fecha_creacion.'</td><td class="center-aligned">'.$conveyor['numero'].'</td><td>'.$conveyor['recommended'].'</td><td class="center-aligned">'.$conveyor['medida'].'</td></tr>';
                        }
                    }else{
                        $data .= '<tr><td colspan="4">'.__('No se encontraron transportadores',true).'</td></tr>';
                    }
