...
<html>
	...
	<?php
                ...
                echo $this->Minify->css(array('site.css','menu','core','forms'));
                echo $this->Minify->css(array('ownPlugins/cocoCropper/cococropper'));
                echo $this->Minify->css(array('ownPlugins/circularMenu/style'));
                ...
                echo $this->Minify->script(array('common_actions','core','common','conti_extensions','phpjs','updates'));
                echo $this->Minify->script(array('ownPlugins/loadingButton/loadingButton','ownPlugins/contiUploader/conti_l10n','ownPlugins/contiUploader/conti.uploader','ownPlugins/cocoCropper/cococropper','ownPlugins/cocoBlock/cocoblock'));
                echo $this->Minify->script(array('ownPlugins/cocoPassGen/cocopass.generator'));
    ...
    <body>                     
        ...
        <div id="container" class="<?php if(isset($responsive) && $responsive){ ?>responsive-page <?php } ?>">    
            ...
            <div id="direct_menu">
                ...
                <!--add items menu -->
                <?php
                    $addCompanyColaboratorAllow = isset($this->_credentials['permissions'][IElement::Is_CompanyColaborator]) && in_array('add', $this->_credentials['permissions'][IElement::Is_CompanyColaborator]['allows']) ? true : false;
                    $addConveyorAllow = isset($this->_credentials['permissions'][IElement::Is_Conveyor]) && in_array('add', $this->_credentials['permissions'][IElement::Is_Conveyor]['allows']) ? true : false;
                    $addNewsAllow = isset($this->_credentials['permissions'][IElement::Is_News]) && in_array('add', $this->_credentials['permissions'][IElement::Is_News]['allows']) ? true : false;
                    $addNotificationsAllow = isset($this->_credentials['permissions'][IElement::Is_Notification]) && in_array('add', $this->_credentials['permissions'][IElement::Is_Notification]['allows']) ? true : false;
                ?>
                <?php if($credentials['role_company']!=UsuariosEmpresa::IS_CLIENT){ ?>
                <ul id="main_saver">
                    <li class="dropdown_menu main-saver">
                        <a class="text-link" href="#" ><?php echo __("v2.5.1_AddElement",true); ?></a>
                        <ul>
                            <span>&nbsp;</span>
                            <?php if($credentials['group_id']>Group::CLIENT && $credentials['role_company']!=UsuariosEmpresa::IS_CLIENT){ ?>
                                <li><?php echo $this->Html->link(__('Empresa', true), '#modal', array('class' => 'add_user', 'alt' => 'company-dialog|callOpenUser', 'rel' => $this->Html->url(array('controller' => 'Users', 'action' => 'add','company')))); ?></li>
                                <li><?php echo $this->Html->link(__('Usuario', true), '#modal', array('class' => 'add_colaborator', 'alt' => 'company-dialog|callOpenUser', 'rel' => $this->Html->url(array('controller' => 'Users', 'action' => 'add','user')))); ?></li>
                            <?php } ?>
