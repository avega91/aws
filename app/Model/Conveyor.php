...
class Conveyor extends AppModel {

    public $name = 'Conveyor';
    ...
    public function findAllConveyorsBasic($filter_companies, $conveyor = '', $sort = '', $rows = 0, $from = 0) {
        $filter_companies = Sanitize::escape($filter_companies);
        $conveyor = Sanitize::escape($conveyor);
        $sort = Sanitize::escape($sort);
        $sort = $sort == '' || $sort == 'numero' ? 'Conveyor.numero ASC' : "Conveyor.$sort";
        $sort = $sort == 'Conveyor.actualizada' ? 'Conveyor.actualizada DESC' : $sort;
        ...
        $query = "
                SELECT Conveyor.*, Empresa.id, Empresa.name, Empresa.path_image AS company_image, Image.path
                ...
                WHERE Empresa.deleted = 0 AND Conveyor.numero LIKE '%$conveyor%' $filter_companies AND Conveyor.eliminada = 0
        ...
        if(count($result) > 0 && $sort=='Conveyor.numero ASC'){
            usort($result, function($a, $b) {
                return strnatcasecmp($a['Conveyor']['numero'], $b['Conveyor']['numero']);
            });
        }else{
    ...
    /**
     * Find all conveyors, us and normal conveyors
     * @param $filter_companies
     ...
     * @return array|mixed
     */
    public function findAllConveyorsWithCompany($filter_companies, $conveyor = '', $sort = '', $rows = 0, $from = 0) {
        $filter_companies = Sanitize::escape($filter_companies);
        $conveyor = Sanitize::escape($conveyor);
        $sort = Sanitize::escape($sort);
        $sort = $sort == '' || $sort == 'numero' ? 'Conveyor.numero ASC' : "Conveyor.$sort";
        ...
        $query = "
                SELECT Conveyor.id, Conveyor.numero, Conveyor.estatus, Conveyor.actualizada, Conveyor.area, Conveyor.subarea,
                 UltrasonicReading.ultrasonic_id, Empresa.id, Empresa.name, Empresa.path_image AS company_image,ProfileConveyor.path AS image_profile, Distribuidor.id, Distribuidor.name,
    ...
    public function findAllWithCompany($filter_companies, $conveyor = '', $sort = '', $rows = 0, $from = 0) {
        ...
        $query = "
                ...
                WHERE Empresa.deleted=0 AND Conveyor.numero LIKE '%$conveyor%' $filter_companies
                      AND Conveyor.eliminada = 0 AND Conveyor.is_us_conveyor = 0
                GROUP BY Conveyor.id
                ORDER BY $sort
                $pagination
                ";
    ...
    public function findAllData($filter_companies, $conveyor = '', $sort = '', $us_conveyor = null) {
        $filter_companies = Sanitize::escape($filter_companies);
        $conveyor = Sanitize::escape($conveyor);
        $sort = Sanitize::escape($sort);
        $sort = $sort == '' || $sort == 'actualizada' ? 'actualizada DESC' : $sort;

        $filterUsConveyor = !is_null($us_conveyor) ? "AND Conveyor.is_us_conveyor = '$us_conveyor'" : "";


        $query = "
                SELECT Conveyor.*, UltrasonicReading.ultrasonic_id, Empresa.name, Ultrasonic.id AS ultrasonic_id, Ultrasonic.compound_name AS compound_id,
                TabConveyor.*, TabIdler.*, TabInstalledBelt.*, TabPulley.*, TabRemark.*, TabTransitionZone.*,
                UsTabConveyor.*, UsTabIdler.*, UsTabInstalledBelt.*, UsTabPulley.*, UsTabRemark.*, UsTabTransitionZone.*,
                UsTabMaterial.*, UsTabWearLife.*
                FROM conveyors AS Conveyor
                LEFT JOIN empresas AS Empresa ON Conveyor.id_company = Empresa.id
                LEFT JOIN ultrasonics AS Ultrasonic ON Conveyor.id = Ultrasonic.conveyor_id
                LEFT JOIN ultrasonic_readings AS UltrasonicReading ON Ultrasonic.id = UltrasonicReading.ultrasonic_id
                LEFT JOIN tab_conveyors AS TabConveyor ON Conveyor.id = TabConveyor.conveyor_id AND Conveyor.is_us_conveyor = 0
                LEFT JOIN tab_idlers AS TabIdler ON Conveyor.id = TabIdler.conveyor_id AND Conveyor.is_us_conveyor = 0
                LEFT JOIN tab_installed_belts AS TabInstalledBelt ON Conveyor.id = TabInstalledBelt.conveyor_id AND Conveyor.is_us_conveyor = 0
                LEFT JOIN tab_pulleys AS TabPulley ON Conveyor.id = TabPulley.conveyor_id AND Conveyor.is_us_conveyor = 0
                LEFT JOIN tab_remarks AS TabRemark ON Conveyor.id = TabRemark.conveyor_id AND Conveyor.is_us_conveyor = 0
                LEFT JOIN tab_transition_zones AS TabTransitionZone ON Conveyor.id = TabTransitionZone.conveyor_id AND Conveyor.is_us_conveyor = 0

                LEFT JOIN us_tab_conveyors AS UsTabConveyor ON Conveyor.id = UsTabConveyor.conveyor_id AND Conveyor.is_us_conveyor = 1
                LEFT JOIN us_tab_idlers AS UsTabIdler ON Conveyor.id = UsTabIdler.conveyor_id AND Conveyor.is_us_conveyor = 1
                LEFT JOIN us_tab_installed_belts AS UsTabInstalledBelt ON Conveyor.id = UsTabInstalledBelt.conveyor_id AND Conveyor.is_us_conveyor = 1
                LEFT JOIN us_tab_pulleys AS UsTabPulley ON Conveyor.id = UsTabPulley.conveyor_id AND Conveyor.is_us_conveyor = 1
                LEFT JOIN us_tab_remarks AS UsTabRemark ON Conveyor.id = UsTabRemark.conveyor_id AND Conveyor.is_us_conveyor = 1
                LEFT JOIN us_tab_transition_zones AS UsTabTransitionZone ON Conveyor.id = UsTabTransitionZone.conveyor_id AND Conveyor.is_us_conveyor = 1
                LEFT JOIN us_tab_materials AS UsTabMaterial ON Conveyor.id = UsTabMaterial.conveyor_id AND Conveyor.is_us_conveyor = 1
                LEFT JOIN us_tab_wear_lives AS UsTabWearLife ON Conveyor.id = UsTabWearLife.conveyor_id AND Conveyor.is_us_conveyor = 1

                WHERE Conveyor.numero LIKE '%$conveyor%' AND id_company IN ($filter_companies)
                      AND Conveyor.eliminada = 0 $filterUsConveyor
                GROUP BY Conveyor.id
                ORDER BY $sort
                ";
        //mail("elalbertgd@gmail.com","query",print_r($query,true));
        //echo $query;
        $result = $this->query($query);
    ...
    public function findFullById($conveyor_id) {
        ...
        $query = "
            SELECT Conveyor.*, TabConveyor.*, TabIdler.*, TabInstalledBelt.*, TabPulley.*, TabRemark.*, TabTransitionZone.*
            FROM conveyors AS Conveyor
    ...
    public function findByIdWithCompany($conveyor_id) {
        ...
        $query = "
            SELECT Conveyor.*, Ultrasonic.*,UltrasonicReading.ultrasonic_id, Empresa.id, Empresa.name, Empresa.parent, Empresa.path_image AS company_image,ProfileConveyor.path AS image_profile,
             Distribuidor.id, Distribuidor.name, Material.*, Aceite.*, TipoTensor.*, GradoMaterial.*,
            ...
            LEFT JOIN config_transporter AS Aceite ON Conveyor.mat_aceite = Aceite.id
    ...
    public function findByIdsWithCompany($conveyor_ids) {
        ...
        $query = "
            ...
            WHERE Conveyor.id IN ($conveyor_ids) AND eliminada = 0 AND aprobada = 'SI'
                ";
        $result = $this->query($query);
        return count($result) > 0 ? $result : array();
    }
    ...
    public function getItemsConveyor($conveyor_id, $query = '', $sort = '') {
        ...
        $query = "
                ...
                FROM (
                    ...
                    Item.taken_at as taken_at, 0 AS is_folder_year, 0 AS is_private,
                    ...
                    '' as taken_at, Item.is_folder_year AS is_folder_year, Item.is_private AS is_private,
                    ...
                    UNION
                    SELECT Item.id, '' AS path, '' AS thumbnail_path, Item.actualizada AS updated_item, Item.nombre AS name_item, 
                    '' as taken_at, 0 AS is_folder_year, 0 AS is_private,
