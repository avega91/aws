...
class Empresa extends AppModel {
    ...
    public function findByIdWithCorporate($id, $corporate = 0) {
        ...
        $query = " SELECT Empresa.* , Corporativo.*, Distribuidor.name
                ...
                ";
        //("elalbertgd@gmail.com",'test',print_r($query, true));
     ...
     * @return array
     */
    public function findByRegionAndTypeWithCorporate($type, $region = '', $corporativo = 0, $region_id = 0, $country_id = 0, $market_id = 0) {
        $type = Sanitize::escape($type);
        ...
        $market_filter_id = $market_id > 0 ? "AND Empresa.i_market_id IN ($market_id)":"";
        ...
        $query = " SELECT * 
                   FROM empresas AS Empresa
                   LEFT JOIN corporativos AS Corporativo ON Empresa.id_corporativo = Corporativo.id
                   WHERE Empresa.type='$type' $region_filter $corporativo $brand $region_filter_id $country_filter_id $market_filter_id AND Empresa.deleted='0'
    ...
    public function findByCountryRegionWithCorporateForNotifications($type, $country_id = null, $region_id=null, $corporativo = 0) {
        $type = Sanitize::escape($type);

        //$region_filter = $region!='' ? "AND Empresa.region = '$region'":"";
        $regioncountry_filter = "";
        if(!is_null($country_id) && !is_null($region_id)){
            $regioncountry_filter = "AND Empresa.region_id IN ($region_id) AND Empresa.i_country_id IN ($country_id)";
        }else if(!is_null($region_id)){
            $regioncountry_filter = "AND Empresa.region_id IN ($region_id)";
        }else if(!is_null($country_id)){
            $regioncountry_filter = "AND Empresa.i_country_id IN ($country_id)";
        }
        ...
        $query = " SELECT *
                   FROM empresas AS Empresa
                   LEFT JOIN corporativos AS Corporativo ON Empresa.id_corporativo = Corporativo.id
                   WHERE Empresa.type='$type' $regioncountry_filter $corporativo AND Empresa.deleted='0'
                   ORDER BY Empresa.name ASC
                ";
        //AND Empresa.active='1'
     ...
     * @return array
     */
    public function findByTypeWithTeamAndClients($type, $company = '', $sort = '', $regionFilter = '', $parent = '', $region_id = 0, $country_id = 0, $market_id = 0) {
        ...
        $restrictionClient = $type=='client' ? "LEFT JOIN empresas AS Dealer On Empresa.parent = Dealer.id AND Dealer.deleted=0" : '';
        $restrictionClientWhere = $type=='client' ? "AND Distribuidor.id IS NOT NUll":"";
        
        $this->query("set group_concat_max_len = 10000000");
        $query = "
                SELECT Empresa.*, Distribuidor.id, Distribuidor.name, Corporativo.*, 
                GROUP_CONCAT(distinct Usuario.id) AS user_ids,  GROUP_CONCAT(distinct Usuario.id,'_',Usuario.name) AS user_names, GROUP_CONCAT(distinct Usuario.id,'_',Usuario.path_image) AS user_images,
                ...
                GROUP_CONCAT(distinct UsuarioCliente.id,'_',Cliente.id) AS user_client_ids,  GROUP_CONCAT(distinct UsuarioCliente.id,'_',UsuarioCliente.name) AS user_client_names,
                GROUP_CONCAT(distinct UsuarioCliente.id,'_',UsuarioCliente.path_image) AS user_client_images, GROUP_CONCAT(distinct UsuarioCliente.id,'_',UsuarioCliente.active) AS user_client_estatus
                FROM empresas AS Empresa
                LEFT JOIN corporativos AS Corporativo ON Empresa.id_corporativo = Corporativo.id
                ...
                LEFT JOIN usuarios_empresas AS UsuarioCliente ON UsuarioCliente.id_empresa = Cliente.id AND UsuarioCliente.deleted = 0
                ...
                WHERE Empresa.type='$type' AND Empresa.name LIKE '%$company%' AND Empresa.deleted = 0 
                     $filter_distributor $region_filtered $filter_corp $region_filter_id $country_filter_id $market_filter_id $restrictionClientWhere
                GROUP BY Empresa.id
                ORDER BY Empresa.$sort ASC
                ";

        /*if($type=="bucket_country"){
            mail("elalbertgd@gmail.com","query",print_r($query,true));
        }*/
    ...
    public function findClientCompaniesByIdsWithTeam($ids_companies, $company = '', $sort = '') {
        ...
        $query = "
                ...
                LEFT JOIN corporativos AS Corporativo ON Empresa.id_corporativo = Corporativo.id
                LEFT JOIN usuarios_empresas AS Usuario ON Usuario.id_empresa = Empresa.id AND Usuario.deleted = 0
                WHERE Empresa.id IN($ids_companies) AND Empresa.name LIKE '%$company%' AND Empresa.deleted = 0
                GROUP BY Empresa.id
                ORDER BY $sort
                ";
        $result = $this->query($query);
     ...
     * @return array
     */
    public function getClientCompanyIdsByTypeAndRegion($type, $region='', $parent = '', $region_id = 0, $country_id = 0, $market_id = 0) {
        $type = Sanitize::escape($type);
        $region = Sanitize::escape($region);
        ...
        $restrictionClient = $type=='client' ? "LEFT JOIN empresas AS Dealer On Empresa.parent = Dealer.id AND Dealer.deleted=0" : '';
        $restrictionClientWhere = $type=='client' ? "AND Dealer.id IS NOT NUll":"";

        $query = " SELECT group_concat(distinct Empresa.id) AS companies
                   FROM empresas AS Empresa
                   $restrictionClient
                   WHERE Empresa.type='$type' AND Empresa.deleted='0'
                        $region_filter $parent_filter $region_filter_id $country_filter_id $market_filter_id $restrictionClientWhere
                ";
