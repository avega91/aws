...
$(document).ready(function() {
    ...
    $(document).on('click', '.toggle-private-link', function(e) {
        e.stopPropagation();
        var link = $(this);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (result.success) {
                    link.addClass(result.class)
                    //link.attr('class', result.class + ' toggle-private-link');
                }
            },
            error: function(xhr, ajaxOptions, thrownError) {
                link.removeClass('processing');
...
function initEventsAddRecommendedBeltInfo(){
    $("#dialog_wrap #recommended_belt_info_form").validationEngine({
    ...
    var $recommended_info_container = $('#recommended_belt_info_conveyor');
    var recommended_belt_info = $recommended_info_container.find('.belt-rec-info').html();
    var recommended_belt_reason = $recommended_info_container.find('.belt-rec-note > span').html();

    $('#dialog_wrap #recommended_belt_info_form .full-controls input#belt_information').val(recommended_belt_info == '-' ? '' : recommended_belt_info);
    $('#dialog_wrap #recommended_belt_info_form textarea[name="reason"]').text(recommended_belt_reason == '-' ? '':recommended_belt_reason);

    $('#dialog_wrap #save_conveyor_belt_info').loadingButton({
        onClick: function(e, btn) {
            e.stopPropagation();
            if ($("#dialog_wrap #recommended_belt_info_form").validationEngine('validate')) {//if ok current section, go to other section
                $.ajax({
                    type: 'post',
                    url: $("#dialog_wrap #recommended_belt_info_form").attr('action'),
                    data: {formdata: $("#dialog_wrap #recommended_belt_info_form").serialize()},
                    success: function(response) {
                        response = $.parseJSON(response);
                        $.coconotif.add({
                            text: response.msg,
...
function initEventsConveyor(){
    ...
    $('#conveyor_details_link, #conveyor_activity, #recommended_belt_details_link').click(function(e) {
        e.stopPropagation();
        $(this).next().slideToggle("slow");
        return false;
    });
    $('#recommended_belt_details_link').trigger('click');

    $('#edit_recommended_belt_info').click(function(e){
        e.stopPropagation();
        $.conti.dialog({html: $('#append_recommended_belt_info_wrapper').html(), style: 'conveyor-dialog', modal: true, callbackOpen: 'initEventsAddRecommendedBeltInfo'});
        return false;
    });
...
function loadInfoRecommendedBelt(){
    $('#edit_recommended_belt_info').addClass('disabled');
    var container = $("#recommended_belt_info_conveyor");
    container.load(jsVars.refreshRecommendedInfoAx + '/' + $("#recommended_belt_info_conveyor").attr('rel'), function(data) {
      $('#edit_recommended_belt_info').removeClass('disabled');
      loadLogConveyors();
    });
}
...
function initItemsConveyorDatatableEvents() {
    ...
    $('.dashboard-list > li:not(.details-item-dashboard):not(.folder-year-item):not(.ultrasonic-item-dashboard):not(.inspection-item-dashboard):not(.item-in-folder) > div').draggable({
        revert: "invalid",
        helper: "clone",
        opacity: 0.75,
        zIndex: 100,
