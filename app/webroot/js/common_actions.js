...
function displayTutorialOnDialog(section){
    ...
    /*f($('#tutorial_toggle').size()>0){ //si esta presente el boton de tutorial
...
function initBuoysDatatableEvents() {
...
function callUpdateConveyorsDataTable() {
    if(typeof jsVars.totConveyors != 'undefined' ){
        console.log(jsVars.totConveyors);
        if (saved_conveyor) {
...
function searchBuoysData(rows, from) {
    ...
    search = search == '' || search == 'Buscar' || search == 'Search' || search == 'Search buoy system by name' ? '-' : search;
    var pages = typeof (rows) !== 'undefined' && typeof (from) !== 'undefined' ? '/' + rows + '/' + from : '';
    var url = jsVars.refreshBuoysAx + '/' + sort + '/' + urlencode(search) + pages;
    ...
    $.get(url, {cid: jsVars.clientCompanyId}, function(data) {
        $('body').cocoblock('unblock');
        if ($("#buoys_data").find('.dashboard-list').size() > 0 && from > 0) {
            $("#buoys_data").find('.dashboard-list').append(data);
            if (from > jsVars.totConveyors) {
                $("#buoys_data").find('> div:last-child').hide();
            } else {
                $("#buoys_data").find('> div:last-child').show();
            }
        } else {
            ...
            $("#buoys_data").find('> div:first-child').html(data);
        }

        initBuoysDatatableEvents();
    }).fail(function() {
        $('body').cocoblock('unblock');
...
function callAddConveyor(dist_id, client_id, active_tab) {
        ...
        var image_portada = $('#reference_bg_uploader').size() > 0 && $('#reference_bg_uploader').val() != '' ? $('#reference_bg_uploader').val() : 'img/img_buoysystem_off.png';
        ...
        $('#save_conveyor').loadingButton({
            onClick: function (e, btn) {
                ...
                processSaveBuoySystem(btn);
                return false;
            }
        });
}

function processSaveBuoySystem(btn){
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                saved_conveyor = true;
                number_saved_conveyor = response.conveyor_number;
                window[function_update_events].apply(null, []); 
            }else{
                $('body').cocoblock('unblock');
            }
        },
...
function initEventsAddFileToFolder() {
    var count=0;
    var fileUploader =  $("#fileuploader").uploadFile({
        url:jsVars.uploadFilesAx,
        uploadStr:"Add files",
        dragDropStr: "<span><b>or Drag &amp; Drop Files Here</b></span>",
        cancelStr: "x",
        fileName:"imodco_files",
        multiple:true,
        showAbort: false,
        dragDrop:true,
        autoSubmit:false,
        allowedTypes: "jpg, jpeg, gif, png, pdf, xls, xlsx, doc, docx, ppt, pptx",
        acceptFiles:".jpg, .jpeg, .gif, .png, .pdf, .xls, .xlsx, .doc, .docx, .ppt, .pptx",
        maxFileSize: 5120 * 1024,
        showFileCounter: false,
        onSelect: function(files){
            setTimeout( function(){
                var errors = $('.ajax-file-upload-error').size();
                if(errors <= 0){
                    $('.ajax-file-upload-container').css('height', '310px');
                }else{
                    var size = errors>1 ? '210px' : '260px';
                    $('.ajax-file-upload-container').css('height', size);
                }
            }
            , 200);
        },
        ...
        afterUploadAll:function(obj){
            ...
            $('#save_file_conveyor').removeClass('uploader-btn');
            $('#save_file_conveyor').loadingButton('stop', 1);
            $('#save_file_conveyor').text('Close');
            $('#fileuploader').fadeOut();
        },
        onSuccess: function (files, response, xhr, pd) {
            pd.progressbar.addClass('successfull-load');
        },
        customProgressBar: function(obj,s)
		{
            ...
            this.abort.addClass("custom-abort");
            this.done.addClass("custom-done");
			this.download.addClass("custom-download");            
            this.cancel.addClass("custom-cancel");
    ...
    $(document).on('dragover','#fileuploader',function(e){
        e.preventDefault();
        $(this).addClass('droppable-section');
      });
