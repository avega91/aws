...
//    var nice = $("html").niceScroll();  // The document page (body)
    ...
    $(document).on('click', '.calculador', function(e) {
        e.stopPropagation();
        var calc_content = $(this).attr('rel');
        var id_calculador = $(this).attr('id');
        ...
        $.ajax({
            ...
            success: function(response) {
                $.conti.dialog({html: response, style: 'tools-dialog', modal: true, callbackOpen: 'initEventsCalculator', paramsOpen: {calculador: id_calculador}});
            },
    ...
    $(document).on('click', '.row-news, .row-news-link', function(e) {
        e.stopPropagation();
        ...
        $.ajax({
            ...
            success: function(response) {
                try {
                    ...
                    $.conti.dialog({html: response, style: 'news-dialog', modal: true});
                }
            },
            error: function(xhr, ajaxOptions, thrownError) {
                errorAjax(xhr, ajaxOptions, thrownError);
    ...
    $(document).on('click', '#generic_add li a:not(.close), .user-profile-link', function(e) {
        e.stopPropagation();
    ...
    $(document).on('click', '.delete-item-link', function(e) {
        ...
        $.conti.confirm({msg: confirmation_msg, type: 'delete-dialog', callbackOk: 'processRemoveItem', paramsOk: {petition: linkPetition, invoker: $(this)}});
        return false;
    });
    ...
    $(document).on('click', '.add-conveyor-link').click(function(e) {
        e.stopPropagation();
        var pet = $(this).attr('rel');
        ...
        $.ajax({
            ...
            success: function(response) {
                try {
                    ...
                    $.conti.dialog({html: response, style: 'conveyor-dialog', modal: true, callbackOpen: 'callAddConveyor', paramsOpen: {dist_id: dealer, client_id: client, active_tab: active_tab}, callbackClose: callback_after_update});
                }
            },
            error: function(xhr, ajaxOptions, thrownError) {
                $('body').cocoblock('unblock');
    ...
    $(document).on('click', '.row-data.active', function(e) {
        e.stopPropagation();
        $('div.clients-distributor').slideUp('normal');
    ...
    $(document).on('click', '.fancy_textarea button:eq(1)', function(e) {
        e.stopPropagation();
        $(this).parent().animate({
            height: '45px'
        }, function() {
            $(this).find('textarea').removeClass('activated');
    ...
    $(document).on('click', '.fancy_textarea button:eq(0)', function(e) {
        e.stopPropagation();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                try {
                    ...
                    if (response.success) {
                        //reload comments  
                        $('.fancy_textarea button:eq(1)').trigger('click');
